...
NB_DIGITAL_PIN.each { |number|
  ...
  if (NB_PWM_PIN).include?(number)
    ...
    digital_pin.push GenericPin.new("/Digital_Pin#{number}",write_ifaces,read_ifaces)
  else
    digital_pin.push GenericPin.new("/Digital_Pin#{number}",write_ifaces,read_ifaces)
  end
  digital_pin[number].set_pin_number(number)
  service.export(digital_pin[number])
}

NB_ANALOG_PIN.each { |number|
  read_ifaces = ["analog"]
  analog_pin.push GenericPin.new("/Analog_Pin#{number}",write_ifaces,read_ifaces)
  analog_pin[number].set_pin_number(number)
